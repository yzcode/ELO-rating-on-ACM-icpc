...
if __name__ == '__main__':
    logging("HDU problem infomation transform start!", 0)
    ...
    sql = "REPLACE INTO problem_info(repo, label,rating,vec) VALUES ('Hdu', '%s', '%s','{}')"
    ...
    sql = "UPDATE problem_info SET vec = '%s' WHERE repo = 'Hdu' and label = %d"
    for i in range(0, vec_count):
        if i % 100 == 0:
            logging("Handling the no.%d problem's vector" % i, 0)
        pro_info = filter(lambda x: len(x) > 1, vec_file.readline().replace('\n', '').split(' '))
        pid = int(pro_info[0])
        pvec = json.dumps(pro_info[1:])
        mysqlcur.execute(sql % (pvec, pid))
    mysqlconn_Local.commit()
    mysqlconn_Local.close()
    logging("HDU problem infomation transform finished!", 0)
